...
let currentFA26Only = false;

function playerHref(name){
  return "player.html?name=" + encodeURIComponent(name);
}

// 🆓-Badge fürs Namensfeld, falls der Spieler in data/fa26-list.json steht
// (Free Agents der NBA-Front-Office-Dynastyliga, siehe js/fa-match.js).
function fa26BadgeFor(name){
  return isFA26(name)
    ? `<span title="Free Agent in der NBA-Front-Office-Liga (FA26)">🆓</span>`
    : "";
}
...
function render(rows){
  ...
  if(currentFA26Only) filtered = filtered.filter(r => isFA26(r.player_name));
...
function renderSticky(rows){
  ...
  body.innerHTML = rows.map((r)=>{
    ...
    return `
    <tr>
      <td style="text-align:left"><a class="player-link" href="${playerHref(r.player_name)}">${r.player_name}</a> ${fa26BadgeFor(r.player_name)} ${sampleBadge}</td>
...
document.getElementById("fa26Filter").addEventListener("change", (e)=>{
  currentFA26Only = e.target.checked;
  if(lastFullRows.length > 0) render(lastFullRows);
});
...
(async function init(){
  ...
  const fa26 = await loadFA26ListFromFile();
  const fa26StatusEl = document.getElementById("fa26Status");
  if(fa26StatusEl){
    fa26StatusEl.textContent = fa26.loaded
      ? `${fa26.count} Free Agents geladen (Stand ${fa26.updated || "unbekannt"}). Neu erzeugen: scripts/update-fa26-list.py.`
      : "Keine FA26-Liste gefunden — Filter/Badge bleiben inaktiv. Siehe scripts/update-fa26-list.py.";
  }
